...
func (b *Bdiscord) Connect() error {
	...
	for _, guild := range guilds {
		...
		if b.guildID != "" {
			return errors.Errorf("There are multiple Discord servers with the same name %#v, expected to see only one", serverName)
		}

		// Getting this guild's channel could result in a permission error
		b.channels, err = b.c.GuildChannels(guild.ID)
		if err != nil {
			return errors.Wrapf(err, "could not get %#v's channels", b.GetString("Server"))
		}
	...
	if b.guildID == "" {
		err = errors.Errorf("Discord server %#v not found", b.GetString("Server"))
